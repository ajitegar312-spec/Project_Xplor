...
export function ThemeToggle({ label }: { label: string }) {
  // `false` initial matches the server render (avoids hydration mismatch);
  // the real state is synced from <html> after mount.
  ...
  useEffect(() => {
    ...
    const onChange = (e: MediaQueryListEvent) => {
      ...
      if (!stored) {
        // No explicit preference: keep the DOM and the icon in sync with the OS.
