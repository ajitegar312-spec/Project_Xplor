...
export default async function AboutPage({ params }: { params: { lang: string } }) {
  ...
  return (
    <>
      <section className="bg-gradient-to-b from-brand-50 to-white py-14" aria-labelledby="about-hero">
        <Container className="max-w-3xl">
          <h1 id="about-hero" className="text-3xl font-extrabold sm:text-4xl">{dict.nav.about}</h1>
      ...
      <section className="mt-14" aria-labelledby="story">
        <Container className="max-w-3xl">
          <h2 id="story" className="text-2xl font-bold">{dict.about.storyTitle}</h2>
          <p className="mt-3 text-slate-600">
            {lang === "id"
      ...
      <section className="mt-14" aria-labelledby="mission">
        <Container className="grid max-w-4xl gap-4 md:grid-cols-2">
          <div className="rounded-2xl border p-6">
            <h2 id="mission" className="text-xl font-bold">{dict.about.missionTitle}</h2>
          ...
          <div className="rounded-2xl border p-6">
            <h2 className="text-xl font-bold">{dict.about.valuesTitle}</h2>
      ...
      <section className="mt-16" aria-labelledby="team">
        <Container>
          <h2 id="team" className="text-2xl font-bold">{dict.about.teamTitle}</h2>
          <ul className="mt-6 grid gap-4 sm:grid-cols-2 lg:grid-cols-4">
            {team.map((m) => (
              <li key={m.name} className="rounded-2xl border p-6 text-center">
                <div className="mx-auto flex h-16 w-16 items-center justify-center rounded-full bg-slate-100 text-xl font-bold" aria-hidden="true">
                  {m.name.split(" ").map((x) => x[0]).slice(0, 2).join("")}
                </div>
                <p className="mt-3 font-bold">{m.name}</p>
                <p className="text-sm text-slate-600">{pick(m.role, lang)}</p>
              </li>
            ))}
          </ul>
        </Container>
